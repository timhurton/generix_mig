CREATE OR REPLACE PROCEDURE "WMS".proc_reference_data()
  LANGUAGE plpgsql AS $$
DECLARE 
  -- BEGIN standard declarations
  vModule "WMS".transform_log.module_name%TYPE := 'PROC_REFERENCE_DATA';
...
BEGIN 
  ...
  -- ***************************************************************************
  vStep := 10;
  vResult := "WMS".transform_log(pModule => vModule

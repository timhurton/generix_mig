...
$install/build_utils_tables.sh > $stdout/build_utils_tables.lst 2> $error_log/build_utils_tables.err
cat $error_log/build_utils_tables.err
$install/build_utils_plpgsql.sh > $stdout/build_utils_plpgsql.lst 2> $error_log/build_utils_plpgsql.err
...
$install/build_plpgsql.sh > $stdout/build_plpgsql.lst 2> $error_log/build_plpgsql.err
cat $error_log/build_plpgsql.err
$install/build_transform_tables.sh > $stdout/build_transform_tables.lst 2> $error_log/build_transform_tables.err
cat $error_log/build_transform_tables.err
$install/build_transform_views.sh > $stdout/build_transform_views.lst 2> $error_log/build_transform_views.err
cat $error_log/build_transform_views.err
psql -d $db -h 10.5.2.4 -p 5432 -U $trfm_user -f $transform/populate_client_parameters.sql > $stdout/populate_client_parameters.lst 2> $error_log/populate_client_parameters.err 
...
psql -d $db -h 10.5.2.4 -p 5432 -U $trfm_user -f $install/build_source_views.sql > $stdout/build_source_views.lst 2> $error_log/build_source_views.err 
cat $error_log/build_source_views.err
psql -d $db -h 10.5.2.4 -p 5432 -U $trfm_user -f $install/build_target_views.sql > $stdout/build_target_views.lst 2> $error_log/build_target_views.err 
